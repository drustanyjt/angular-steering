import argparse
from tsad_utils import get_tsad_instructions
...
from utils import tokenize_instructions_fn, add_hooks, get_mlp_input_hook, get_residual_hook
...
def extract_activations(
    ...
    with add_hooks(module_forward_pre_hooks=pre_hooks, module_forward_hooks=forward_hooks):
        with torch.no_grad():
            for i in tqdm(
    ...
    for key, value in cache.items():
        activations[key] = value.squeeze(1)
    return activations

def compute_steering_directions(harmful_acts: dict, harmless_acts: dict, strategy: str = "both"):
    ...
    all_candidates = torch.stack([candidate_directions[key] for key in sorted(candidate_directions.keys(), key=sort_key)])
    ...
    if strategy in ["max_sim", "both"]:
        ...
        candidates_stack = torch.stack([candidates_normalized[key] for key in sorted(candidates_normalized.keys(), key=sort_key)])
...
def save_steering_configs(directions, layers, output_path, args):
    ...
    for strategy, config in directions.items():
        ...
        logger.info(f"  Saved: {filename} (best: layer {best_layer_idx}, {len(config_all_layers)} module entries)")
...
def main():
    ...
    parser.add_argument("--positions", type=str, nargs="+", default=["mid", "post"])
    parser.add_argument("--strategy", type=str, default="both", choices=["max_sim", "max_norm", "both"])
    parser.add_argument("--language", type=str, default="en")
    args = parser.parse_args()

    model_name = args.model.split("/")[-1]
    output_path = f"{args.output_dir}/{model_name}"
    import os
    ...
    logger.info(f"\nLoading TSAD dataset...")
    harmful_train, _ = get_tsad_instructions("harmful", args.n_samples)
    harmless_train, _ = get_tsad_instructions("harmless", args.n_samples)
    logger.info(f"Using {len(harmful_train)} harmful and {len(harmless_train)} harmless samples")
    logger.info("\nExtracting harmful activations...")
    harmful_acts = extract_activations(model, harmful_train, tokenizer, layers, args.positions, args.batch_size)
    import gc
    gc.collect(); torch.cuda.empty_cache()
    logger.info("\nExtracting harmless activations...")
    harmless_acts = extract_activations(model, harmless_train, tokenizer, layers, args.positions, args.batch_size)
    gc.collect(); torch.cuda.empty_cache()
    logger.info("\nComputing steering directions...")
    directions = compute_steering_directions(harmful_acts, harmless_acts, args.strategy)
    save_steering_configs(directions, layers, output_path, args)
