...
def compute_steering_directions(
    positive_acts: dict, negative_acts: dict, strategy: str = "both"
):
    ...
    for key in positive_acts.keys():
        positive = positive_acts[key].float()  # (batch, hidden_dim) - convert to float32
        negative = negative_acts[
            key
...
def main():
    ...
    positive_train, _ = get_input_data("positive", args.language)
    negative_train, _ = get_input_data("negative", args.language)

    positive_train = positive_train[: args.n_samples]
    negative_train = negative_train[: args.n_samples]

    logger.info(
        f"Using {len(positive_train)} positive and {len(negative_train)} negative samples"
    )

    # Extract activations
    logger.info("\nExtracting harmful activations...")
    positive_acts = extract_activations(
        model, positive_train, tokenizer, layers, args.positions, args.batch_size
    )
    # Clear cache
    gc.collect()
    ...
    negative_acts = extract_activations(
        model, negative_train, tokenizer, layers, args.positions, args.batch_size
    )
    # Clear cache
    gc.collect()
    torch.cuda.empty_cache()
    ...
    directions = compute_steering_directions(positive_acts, negative_acts, args.strategy)
